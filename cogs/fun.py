...
from functions import last_replace , text_to_owo
import asyncio
...
melon = disnake.Color.from_rgb(248,131,121)
...
class Fun(commands.Cog):
    def __init__(self, bot):
        ...
        if REDDIT_APP_ID and REDDIT_APP_SECRET:
            self.reddit = praw.Reddit(client_id=REDDIT_APP_ID, client_secret=REDDIT_APP_SECRET,
                                      user_agent="ULTIMATE_disnake_BOT:%s:1.0" % REDDIT_APP_ID)
    ...
    async def eightball(self, ctx, *, _ballInput: commands.clean_content):
        ...
        if choiceType == "(Affirmative)":
            ...
            emb = (disnake.Embed(title="Question: {}".format(_ballInput), colour=0x3be801, description=prediction))
            ...
            emb = (disnake.Embed(title="Question: {}".format(_ballInput), colour=0xff6600, description=prediction))
            ...
            prediction = choice(["Don't count on it ", 
                                        ...
                                        "Outlook not so good ", 
                                        "Very doubtful "]) + ":8ball:"
            emb = (disnake.Embed(title="Question: {}".format(_ballInput), colour=0xE80303, description=prediction))
    ...
    async def gay_scanner(self, ctx,* ,user: commands.clean_content=None):
        ...
        if gayness <= 33:
            ...
            gayStatus = choice(["LOL YOU GAY XDDD FUNNY", 
                                       ...
                                       "HELLA GAY"])
            gayColor = 0xFF00FF
        emb = disnake.Embed(description=f"Gayness for **{user}**", color=gayColor)
            ...
            if self.reddit:
                ...
                while True:
                    ...
                    if url.endswith("jpg") or url.endswith("png"):
                        ...
                        randomsub = choice(allsubs)
                em = disnake.Embed(title=name, color=disnake.Color.blurple())
    ...
    @commands.cooldown(rate=1, per=5, type=commands.BucketType.default)
    async def insult(self, ctx, member: disnake.Member = None):
    ...
    async def dog(self, ctx):
        ...
        embed = disnake.Embed(title="Doggo!", color=disnake.Color.purple(), description=factjson['fact'])
    ...
    async def cat(self, ctx):
        ...
        embed = disnake.Embed(title="Cat", color=disnake.Color.purple(), description=factjson['fact'])
        embed.set_image(url=dogjson['link'])
        await ctx.send(embed=embed)
    ...
    async def gif(self, ctx,*,q="Anime"):
        ...
        try:
            ...
            rgiff = choice(lst)
            emb = disnake.Embed(color=ctx.author.color)
    ...
    @commands.cooldown(rate=1, per=5, type=commands.BucketType.default)
    async def poke(self, ctx, member: disnake.Member = None):
    ...
    async def urban(self,ctx,*,define):
        ...
        ex = term[0]['example']
        meaning = term[0]['def']
        embed = disnake.Embed(title=f"{define.title()}",description=f"Meaning : {meaning}\n {term[1]['def']}",color=melon)
