...
def ButHelp(ctx,cog, desc):
    embed = disnake.Embed(description=f"```diff\n- <> = optional argument\n- [] = required argument```")
...
def getDesc(ctx):
    prefix = guild.check_prefix(ctx.guild.id)
    descDic = {}
    descLi = []
    pages = []
    desc = f""
...
class MyHelp(commands.HelpCommand):
    async def send_bot_help(self, mapping):
        prefix = guild.check_prefix(self.context.guild.id)
        embed = disnake.Embed()
        ...
        btn1 = Button(label="Invite", style=disnake.ButtonStyle.url, url="https://disnake.com/api/oauth2/authorize?client_id=850243448724127754&permissions=140076435526&scope=bot", emoji=disnake.PartialEmoji(name="invite", id=939483316367802418))
        btn3 = Button(label="Support Server", style=disnake.ButtonStyle.url, url="https://disnake.gg/eZFKMmS6vz", emoji=disnake.PartialEmoji(name="head", id=939483469426360400))
        btn2 = Button(label="Vote Me", style=disnake.ButtonStyle.url, url="https://top.gg/bot/850243448724127754/vote", emoji=disnake.PartialEmoji(name="head", id=939483469426360400))
        view.add_item(btn1)
        view.add_item(btn3)
        view.add_item(btn2)
    ...
    async def send_command_help(self, command):
        ...
        embed = disnake.Embed(title=signature, description=command.description or "No help found...",color=self.context.bot.user.color)
    ...
    async def send_help_embed(self, title, description, commands): # a helper function to add commands to an embed
        embed = disnake.Embed(title=title, description=description or "No help found...")
