...
from database import guild as guildprefix, user as userdb
...
options = [
            disnake.SelectOption(
                label="Partner", description="Check your Marital Status.", emoji=disnake.PartialEmoji(name="amyChibiILY", id="863681666673147914")
            ),
            disnake.SelectOption(
                label="Siblings", description="Check Your Siblings.", emoji=disnake.PartialEmoji(name="amyBlush", id="898532188813950986")
            )
        ]
...
class FamilyOption(View):
    ...
    @disnake.ui.select(placeholder="Choose an option.",min_values=1,max_values=1,options=options)
    async def callback(self,selection : disnake.ui.Select , interaction: disnake.Interaction):
        ...
        if self.user.id:
            ...
            if selection.values[0].lower() == "siblings":
                ...
                if str(rel) == "0":
                    ...
                    ppl = disnake.utils.get(self.bot.get_all_members(), id=int(rel))
                    desc= f"`[1] {ppl.name}`"
                    lensib=f"{len(sib)}"
                else:
                    for a in range(len(sib)):
                        ...
                        try:
                            print(sib[a])
                            ppl = disnake.utils.get(self.bot.get_all_members(), id=int(sib[a]))
                            desc = desc + f"`{ppl.name}` | "
                        except Exception as e:
                            print(e)
                embed=disnake.Embed(color=self.user.color,description=f"** Siblings[{lensib}] :**\n{desc}")
                ...
                if partner == 0:
                    emb = disnake.Embed(description=f"{self.user.name}, you are not married! Please Marry Someone First! \n Usage : `{prefix}marry `<user>``", color=0xf72585)
                    ...
                    prsn = disnake.utils.get(self.bot.get_all_members(), id = partner)
                    emb = disnake.Embed(description=f"Married since {dat} ({diff.days} days) ! The Perfect Cuple <3 UwU.\nYou've dated `{dates}` times ~Damn!\n> And Your UwU score is `{scores}`! Pretty Good :smirk:", color=0xf72585)
...
class MarryButton(View):
    ...
    @disnake.ui.button(style=disnake.ButtonStyle.success, emoji="<:amyYes:913690556452974592>", label="Yes !!")
    async def yesButton(
    ...
    @disnake.ui.button(style=disnake.ButtonStyle.danger, emoji="<:pepe_no:848275072435224586>", label="No ;-;")
    async def cancel(self, button: Button, interaction: disnake.Interaction):
        user1 = interaction.user.id
        if self.user.id == user1:
            # await interaction.response.send_message("Confirming", ephemeral=True)
            self.value = False
...
class Family(commands.Cog):
    ...
    async def marry(self, ctx, user: disnake.Member=None):
        ...
        if user is None:
            ...
            if partner == 0:
                emb = disnake.Embed(description=f"{ctx.author.name}, you are not married! Please Marry Someone First! \n Usage : `{prefix}marry `<user>``", color=0xf72585)
            ...
            if ctx.author.id == user.id:
                emb = disnake.Embed(title=f"", description=f"> You Can't Marry Yourself Dumbo!\n**__Usage__:**\n`{prefix}marry `<user>``", color=0xf72585)
            ...
            if userdb.is_sibling(ctx.author.id, user.id):
                emb = disnake.Embed(title=f"", description=f"> You Can't Marry Your Sibling !!\nI mean u can- But my dev won't allow it.\n**__Usage__:**\n`{prefix}marry `<user>``", color=0xf72585)
            ...
            if prsn1 == 0 and prsn2 == 0:
                emb = disnake.Embed(description=f"Hey, {user.mention}, it would make {ctx.author.mention} really happy if you would marry them. What do you say?\n", color=0xf72585)
                emb.set_author(name=f"{ctx.author.name} has proposed to {user.name}! <3 ")
                emb.timestamp = datetime.utcnow()



                btn3= Button(style=disnake.ButtonStyle.blurple, emoji="<:amySad1:913697522474704916>", label="TIMEUP", disabled=True)
                ...
                try:
                    ...
                    if view.value is None:
                        ...
                        emb = disnake.Embed(f"😠 || Sorry, {ctx.author.mention}, {user.name} didn't reply on time!! Maybe They are confused, lets give them some time!", color=0xf72585)
                        emb.timestamp = datetime.utcnow()
                        ...
                        emb = disnake.Embed(description=f"💒 || {ctx.author.mention} and {user.mention} are now married! Congrats!!", color=0xf72585)
                        emb.timestamp = datetime.utcnow()
                        ...
                        view2= View()
                        await msg.edit(view=view2)
                        emb = disnake.Embed(f"💔 || {user.mention}, you have decline a marriage request to {ctx.author.mention}", color=0xf72585)
                        emb.timestamp = datetime.utcnow()
                ...
                emb = disnake.Embed(f":no_entry_sign: | **{ctx.author.name}** , you or your friend is already married!", color=0xf72585)
                emb.timestamp = datetime.utcnow()
                await ctx.send(embed=emb)
    ...
    async def div(self,ctx):
        ...
        if partner == 0:
            ...
            try:
                ppl = disnake.utils.get(self.bot.get_all_members(), id=partner)
                ...
                emb=disnake.Embed(description=f"You married on {dat} and have been married for {diff.days} days... Once you divorce, the ring will break and disappear.")
    ...
    async def sibling(self, ctx, user: disnake.Member=None):
        ...
        if user == None:
            ...
            desc = ""
            if rel == "0":
                emb=disnake.Embed(description=f"{ctx.author.name}, You have no E-siblings right now! Please make Someone First! ex. `{prefix}sib `<user>`` ", color=0xf72585)
            ...
            if len(sib) == 1:
                ppl = disnake.utils.get(self.bot.get_all_members(), id=int(rel))
                emb=disnake.Embed(description=f"▪️ {ppl.name} `({ppl.id})`", color=0xf72585)
        ...
        if ctx.author.id ==user.id:
            ...
            if userdb.is_partner(ctx.author.id, user.id):
                emb = disnake.Embed(title=f"", description=f"> If You wanna make your partner your sibling, First take a divorce!\n**__Usage__:**\n`{prefix}div `<user>``")
            ...
            if rel == "0" and rel2=="0" or str(user.id) not in sib and str(ctx.author.id) not in sib2 :
                emb = disnake.Embed(description=f"Hey {user.mention},  I feel glad too say that {ctx.author.mention} wants to make you their sibling ! It would make them  really happy if you accept this proposal. What do you say?\n")
                emb.set_author(name=f"{ctx.author.name} wants to make {user.name} their Sibling! <3 ")
                emb.timestamp = datetime.utcnow()
                emb.set_thumbnail(url="https://cdn.disnakeapp.com/attachments/865570883170861077/865875381537734686/output-onlinegiftools.gif")
                emb.set_footer(icon_url=self.bot.user.avatar.url)
    ...
    @commands.command(aliases=["nosib"], brief="Disown Your Sister.",usage="`<user>`",name="disown",help="Disown Your Sister.", extras={"emoji": "😭"},description="Disown Your Sister.")
    @commands.cooldown(rate=1, per=5, type=commands.BucketType.default)
    async def remvsib(self,ctx, user: disnake.Member=None):

        prefix = guildprefix.check_prefix(ctx.guild.id)
        rel = userdb.showSib(ctx.author.id).split(',')
        if len(rel) == 0:
        ...
        if user == None:
            ...
            try:
                if str(user.id) in rel:
                    emb=disnake.Embed(description=f"{ctx.author.mention}, Are you sure? ", color=disnake.Color.nitro_pink())
    ...
    @commands.cooldown(rate=1, per=5, type=commands.BucketType.default)
    async def family(self,ctx, user: disnake.Member=None):

        prefix = guildprefix.check_prefix(ctx.guild.id)
        if user == None:
            view = FamilyOption(self.bot, ctx.author)
            emb = disnake.Embed(color=ctx.author.color,description=f"Nickname: `{ctx.author.name}`\nID: `{ctx.author.id}`")
            ...
            if userdb.is_profile_enabled(user.id):
                ...
                emb = disnake.Embed(color=user.color,description=f"Nickname: `{user.name}`\nID: `{user.id}`")
    ...
    async def date(self,ctx):
        ...
        if not userdb.is_married(ctx.author.id):
            emb = disnake.Embed(description=f"{ctx.author.name}, you are not married! In Order to get UwU points ,Please Marry Someone First! \n Usage : `{prefix}marry `<user>``")
            emb.set_author(name=f"You are not currently married.", icon_url=ctx.author.avatar.url)
            return await ctx.channel.send(embed=emb)
        else:
            if not userdb.date_today(ctx.author.id) or userdb.date_today(part):
                emb = disnake.Embed(description=f"💰 | {ctx.author.mention}, Here is your daily for your date `{money}`\n💞 | You've dated {dates} times!\nAnd Your UwU score is `{scores}`! Pretty Good :smirk:")
                await ctx.channel.send(embed=emb)
    ...
    async def mlb(self, ctx, *, arg: str = None):
        ...
        if arg == None:
            ...
            emb = disnake.Embed(description="Commands Available:\n> `mlb guild` for server marriage leaderboard.\n> `mlb global` for global marriage leaderboard.", color=disnake.Color.red())
            emb.set_author(
