...
class General(commands.Cog):
    ...
    async def av(self, ctx, m1: disnake.Member = None, m2: disnake.Member = None):
        ...
        if m1 == None and m2 == None:
            ...
            if m1.avatar.is_animated():
                ...
                file = disnake.File("avatar.gif")
                embed = disnake.Embed(title="Avatar" ,color=0xe91e63,description=f"`Nickname: {m1.display_name}\nID: {m1.id}`")
        ...
        #for shared pfp
            ...
            if m1.avatar.is_animated() and m2.avatar.is_animated():
                ...
                if c == 0:
                    ...
                    file = disnake.File('shared.gif')
                    embed = disnake.Embed(title="Avatar" ,color=0xe91e63)
    ...
    async def serverav(self, ctx,m1: disnake.Member = None ):
        '''Check Server Specific Avatar.'''
        if m1 is None:
            m1 = ctx.author
        embed = disnake.Embed(color=0xe91e63)
        embed.set_author(name=ctx.author, icon_url=ctx.author.avatar.url)
        embed.timestamp = datetime.datetime.utcnow()
    ...
    async def banner(self, ctx,m1: disnake.Member = None ):
        ...
        for word in cont:
            if word.startswith("<") and word.endswith(">"):
                ...
                if word.startswith("<a:"):
                    emoj = disnake.PartialEmoji(name=lst[0], id=lst[1], animated=True)
                else:
                    emoj = disnake.PartialEmoji(name=lst[0], id=lst[1])
                asset = emoj.url
                emb = disnake.Embed(description=f"`{lst[1]}`\n`{lst[0]}`", color=0x2e69f2)
                emb.set_author(
    ...
    #         emb = disnake.Embed(description=f"Removed {channel} from blacklisted channels.")
    #         await ctx.channel.send(embed=emb)
    #     elif not afk.is_channel_blacklisted(ctx.guild.id, channel.id):
    #         afk.add_blacklist(ctx.guild.id, channel.id)
    #         emb = disnake.Embed(description=f"Added {channel.id} to blacklisted channels.")
    #         await ctx.channel.send(embed=emb)
    #     else:
    #         return
