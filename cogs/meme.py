...
class Meme(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.hidden = False
        self.description = "Includes Image Related Commands."



    @commands.command(usage=f"`< user >`", description=f"Simp Overlay")
    async def simp(self, ctx, m1:disnake.Member=None):
        '''Simp Overlay'''
        if m1 is None:
            m1=ctx.author
        ...
        file = disnake.File(f"generated.png", filename="pic.jpg")
        emb = disnake.Embed(title="", description=f"", color=0xe91e63)
        emb.set_image(url="attachment://pic.jpg")
        await ctx.send(file=file, embed=emb)

    @commands.command(description=f"Ship Two User.", usage=f"`< user1 >` `< user2 >`")
    async def ship(self, ctx, m1:disnake.Member=None, m2:disnake.Member=None):
    ...
    async def amogus(self, ctx, m1:disnake.Member=None, m2:disnake.Member=None):
        '''Among US Meme.'''
        if m1 == None or m2 == None:
            return
    ...
    # @commands.command()
    # async def jojoshock(self, ctx, m1:disnake.Member=None):
    #     '''Jojo Meme'''
    #     if m1 == None :
    #         m1=ctx.author
    ...
    async def eject(self, ctx, m1:disnake.Member=None):
        ...
        file = disnake.File(f"generated.gif", filename="pic.gif")
        emb = disnake.Embed(title="", description=f"", color=0xe91e63)
        emb.set_image(url="attachment://pic.gif")
        await ctx.send(file=file, embed=emb)

    @commands.command(usage=f"`< user >`", description=f"Pet Pet Generator.")
    async def petpet(self, ctx, m1:disnake.Member=None):
        '''Pet Pet Generator.'''
        if m1 == None :
            m1=ctx.author
        ...
        dest.seek(0)
        file = disnake.File(dest, filename="pat.gif")
        emb = disnake.Embed(description=f"`{m1.name.lower()}_pat`",color=0x2e69f2)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://some-random-api.ml/canvas/triggered?avatar={member.avatar.with_format("png").url}') as af:
                if 300 > af.status >= 200:
                    ...
                    file = disnake.File(fp, "triggered.gif")
                    em = disnake.Embed(
                        color=0xf1f1f1,
                    )
    ...
    async def horny(self,ctx, member: disnake.Member = None):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://some-random-api.ml/canvas/horny?avatar={member.avatar.with_format("png").url}') as af:
                if 300 > af.status >= 200:
                    fp = BytesIO(await af.read())
                    file = disnake.File(fp, "horny.png")
                    em = disnake.Embed(
                        color=0xf1f1f1,
                    )
    ...
    async def gaycard(self, ctx, member: disnake.Member=None):
        '''You are Gae Bro!'''
        member = member or ctx.author
        await ctx.trigger_typing()
        async with aiohttp.ClientSession() as session:
    ...
    async def wasted(self, ctx, member: disnake.Member=None):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://some-random-api.ml/canvas/wasted?avatar={member.avatar.with_format("png").url}') as af:
                if 300 > af.status >= 200:
                    fp = BytesIO(await af.read())
                    file = disnake.File(fp, "wasted.gif")
                    em = disnake.Embed(
    ...
    async def mission(self, ctx, member: disnake.Member=None):
        '''Mission Passed'''
        member = member or ctx.author
        await ctx.trigger_typing()
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://some-random-api.ml/canvas/mission?avatar={member.avatar.with_format("png").url}') as af:
                if 300 > af.status >= 200:
                    fp = BytesIO(await af.read())
                    file = disnake.File(fp, "mission.gif")
                    em = disnake.Embed(
    ...
    async def comrade(self, ctx, member: disnake.Member=None):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://some-random-api.ml/canvas/comrade?avatar={member.avatar.with_format("png").url}') as af:
                if 300 > af.status >= 200:
                    fp = BytesIO(await af.read())
                    file = disnake.File(fp, "comrade.gif")
                    em = disnake.Embed(
    ...
    async def simpcard(self,ctx, member: disnake.Member = None):
        '''Simpcard just for u'''
        member = member or ctx.author
        await ctx.trigger_typing()
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://some-random-api.ml/canvas/simpcard?avatar={member.avatar.with_format("png").url}') as af:
                if 300 > af.status >= 200:
                    fp = BytesIO(await af.read())
                    file = disnake.File(fp, "simpcard.png")
                    em = disnake.Embed(
    ...
    async def lolice(self,ctx, member: disnake.Member = None):
        '''TRY THIS.'''
        member = member or ctx.author
        await ctx.trigger_typing()
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://some-random-api.ml/canvas/lolice?avatar={member.avatar.with_format("png").url}') as af:
                if 300 > af.status >= 200:
                    fp = BytesIO(await af.read())
                    file = disnake.File(fp, "lolice.png")
                    em = disnake.Embed(
